...
When MCP data sources are connected, validate goal targets against historical baselines. Skip if tools are unavailable.
...
- Segment-specific baselines (filter by segmentId for cohort-level targets)

This replaces estimation with actual data for Conservative/Ambitious/Moonshot target calibration.

### CSAT/Satisfaction Baselines from Zendesk

For satisfaction-related goals:
```sql
...
### PES Component Trends

Use `mcp__claude_ai_Pendo__productEngagementScore` with a recent date range to get current PES and component scores (adoption, stickiness, growth). Compare against target goals to validate feasibility — e.g., if current growth score is 30 and goal implies 80, that's likely Moonshot territory.
