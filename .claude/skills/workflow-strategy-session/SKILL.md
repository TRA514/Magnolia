---
name: workflow-strategy-session
...
### 3b. Cross-Dataset Context Check (qmd)

After gathering research and meeting signals, run a qmd cross-dataset check to surface
any existing product decisions and open tasks related to the decision question.

```bash
# Check for existing product decisions
qmd search "<decision_question_keywords>" -c product_artifacts --json -n 5

# Check for open tasks related to this topic
qmd search "<decision_question_keywords>" -c tasks --json -n 5
```

**If results found, append to `{session-folder}/context.md`:**

```markdown
## Existing Product Context

### Related Product Decisions
[List PRDs, epics, or roadmap items surfaced from product_artifacts]
- **[Title]**: [Snippet — key decision or scope from this artifact]
  - Path: [file path]

### Open Tasks on This Topic
[List any in-flight tasks related to the decision]
- **[Task Title]** ([queue], [priority]): [snippet]
  - Path: [file path]

> These existing commitments should inform the strategy — avoid decisions that conflict
> with in-flight work unless explicitly superseding it.
```

**Skip if qmd unavailable.** Do not block the session if this step fails.

### Step 3c: MCP Live Data Context (Optional)
...
**Topic-specific enrichment by session type**:
- `competitive-analysis`: Add Pendo Listen with `feedbackTypes: ["Competitor Weakness", "Competitor Strength"]` and Gong competitive tracker query
- `pricing-strategy`: Add Pendo Listen with `similaritySearchTerms: ["pricing", "cost", "value", "expensive"]`
- `product-strategy`: Add Pendo PES scores via `productEngagementScore` and feature adoption data
- `market-positioning`: Add Zendesk sentiment trends and Pendo Listen feedback volumes by account type
- `customer-segmentation`: Add Pendo visitor/account counts by segment via `segmentList` + `accountQuery`
- `growth-strategy`: Add Pendo growth score trend and Zendesk churn-risk feedback via `get_feedback_items` with alerts: ["Churn Risk"]
