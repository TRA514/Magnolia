---
name: workflow-tradeoff-decision
...
When MCP data sources are connected, inform tradeoff analysis with segment-level data and UX evidence. Skip if tools are unavailable.

### Segment-Level Impact Analysis (Pendo)
...
Session replay evidence can break deadlocks: if users show clear frustration patterns, that tips the scale toward "Iterate First" even if aggregate metrics look acceptable.

### Support Impact Check (Zendesk)

Check whether the test period correlates with support ticket changes:
```sql
...
A spike in related support tickets during the test period is strong evidence of negative customer impact, even if product metrics look neutral.
