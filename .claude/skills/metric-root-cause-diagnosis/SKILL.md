---
name: metric-root-cause-diagnosis
...
## MCP-Assisted Investigation (Optional)

When MCP data sources are connected, auto-query external systems instead of relying solely on stakeholder interviews. Skip if tools are unavailable.

### People & Technology Dimensions (Pendo)
...
### Internal Factors (Azure DevOps)

Check for recent releases that may correlate with the metric change:
...
### External Factors (Gong + Zendesk)

**Competitive signals from Gong**:
```sql
...
**Support ticket volume trends** (detect spikes correlating with metric change):
```sql
...
**Pendo Listen alerts**: Use `mcp__claude_ai_Pendo__get_feedback_items` with `alerts`: ["Churn Risk", "High Frustration"] for the investigation period to surface acute customer pain signals.

### Integration with Investigation Flow

Map MCP findings to the hypothesis table:
- Pendo segment data → People/Technology dimension narrowing
- Azure DevOps releases → Internal factor hypotheses
- Gong competitive mentions → External factor hypotheses
- Zendesk ticket spikes → Corroborating evidence for timing
- Pendo session replays → UX-related root cause evidence
