---
name: metric-proxy-metric-selection
...
## Correlation Validation with Live Data (Optional)

When Pendo MCP is connected, validate proxy metric selection against actual product data. Skip if tools are unavailable.
...
- Compare 30-90 day time series of proxy vs. outcome to check directional alignment
...
If the proxy behavior isn't instrumented in Pendo, flag this as a prerequisite: "Pendo instrumentation needed for {proxy metric} before measurement can begin."

### Counter-Metric Monitoring Sources

For each identified counter-metric, check if monitoring data exists:
- **Pendo**: Search for pages/features related to the counter-metric behavior
- **Zendesk**: Check if ticket volume for the related product area can serve as a counter-metric signal:
  ...
  ORDER BY week
  ```
- **Pendo CSAT/NPS**: Use `mcp__claude_ai_Pendo__activityQuery` with `entityType: "poll"` to check for existing NPS/satisfaction surveys that could serve as counter-metric proxies.
