...
When MCP data sources are connected, supplement estimation-based scores with actual data. Skip if tools are unavailable.

### Impact Scoring Enhancement
...
Supplement meeting-based frequency with external signal sources:
...
- **Zendesk ticket volume**:
  ...
  ```
  Add ticket count to FrequencyScore calculation.
...
- **Gong tracker mentions**:
  ...
  ```
  Add Gong mentions to FrequencyScore calculation.

- **Pendo Listen feedback**: Use `mcp__claude_ai_Pendo__get_feedback_items` with `similaritySearchTerms` matching the feature area to count feedback submissions. Add to DiversityScore as an additional source type.

### Scoring with MCP Data

When MCP data is available, the enhanced scoring formula weights signals from more source types:
- **DiversityScore**: Meetings + Pendo Listen + Gong + Zendesk = 4 source types → DiversityScore 10.0
- **FrequencyScore**: Sum of meeting mentions + Zendesk tickets + Gong tracker mentions
- **Impact**: Replace estimation with actual Pendo adoption percentage when available
